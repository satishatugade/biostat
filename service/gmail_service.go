...
func (s *GmailSyncServiceImpl) FetchEmailsWithAttachment(service *gmail.Service, userId uint64, filterString string, processID uuid.UUID) ([]*models.TblMedicalRecord, error) {
	...
	s.processStatusService.LogStep(processID, step, constant.Running, msg1, errorMsg, nil, nil, nil, nil)
	...
	if err != nil {
		...
		s.processStatusService.LogStepAndFail(processID, step, constant.Failure, string(constant.GmailSearchMessage), err.Error())
		return nil, err
	}
	s.processStatusService.LogStep(processID, step, constant.Success, msg1, errorMsg, nil, nil, nil, nil)
	...
	for idx, msg := range results.Messages {
		...
		msg := fmt.Sprintf("Processing email attachment: %d / %d", idx+1, len(results.Messages))
		s.processStatusService.LogStep(processID, findMailstep, constant.Running, msg, errorMsg, nil, nil, nil, nil)
	...
	s.processStatusService.LogStep(processID, findMailstep, constant.Success, msg3, errorMsg, nil, &totalRecord, nil, nil)
...
func (s *GmailSyncServiceImpl) ExtractAttachment(service *gmail.Service, message *gmail.Message, userEmail string, userId uint64, processID uuid.UUID, mailIdx int) []*models.TblMedicalRecord {
	...
	for idx, part := range message.Payload.Parts {
		if part.Filename != "" {
			...
			log.Println("@ExtractAttachments Processing Record from Email:", mailIdx, "-", count, "/", len(message.Payload.Parts), ": ", part.Filename, "-", getHeader(message.Payload.Headers, "Subject"))
			s.processStatusService.LogStep(processID, step, constant.Running, msg, errorMsg, nil, &count, nil, nil)
			s.processStatusService.LogStep(processID, step1, constant.Running, msg1, errorMsg, nil, &count, nil, nil)
			...
			if err != nil {
				...
				s.processStatusService.LogStepAndFail(processID, step1, constant.Failure, string(constant.CheckDocTypeFailedMessage), err.Error())
			...
			s.processStatusService.LogStep(processID, step, constant.Running, msg, errorMsg, nil, &count, nil, nil)
			s.processStatusService.LogStep(processID, step1, constant.Running, newmsg, errorMsg, nil, &count, nil, nil)
		}
	}
	count := len(records)
	failedCount := totalAttempted - successCount
	s.processStatusService.LogStep(processID, step, constant.Success, string(constant.DownloadAttachmentComplete), errorMsg, nil, &count, &successCount, &failedCount)
	s.processStatusService.LogStep(processID, step1, constant.Success, docCompleteMsg, errorMsg, nil, &totalAttempted, &successCount, &failedCount)
	return records
}
...
func (gs *GmailSyncServiceImpl) GmailSyncCore(userId uint64, processID uuid.UUID, gmailService *gmail.Service) error {
	...
	errorMsg := ""
	gs.processStatusService.LogStep(processID, step, constant.Success, msg, errorMsg, nil, nil, nil, nil)
	labs, err := gs.diagnosticRepo.GetPatientLabNameAndEmail(userId)
	if err != nil {
		gs.processStatusService.LogStepAndFail(processID, step, constant.Failure, string(constant.UserLabNotFound), err.Error())
		log.Println("@GmailSyncCore->GetPatientLabNameAndEmail:", userId, " err:", err)
		return err
	} else {
		gs.processStatusService.LogStep(processID, step, constant.Success, string(constant.UserLabFetched), errorMsg, nil, nil, nil, nil)
	}
	filterString := utils.FormatLabsForGmailFilter(labs)
	profile, err := gmailService.Users.GetProfile("me").Do()
	if err != nil {
		step := string(constant.ProcessVerifyCredentials)
		gs.processStatusService.LogStep(processID, step, constant.Success, string(constant.InvalidCredentials), errorMsg, nil, nil, nil, nil)
		log.Println("@GmailSyncCore->gmailService.Users.GetProfile:", userId, " err:", err)
		return err
	}
	step1 := string(constant.ProcessFetchEmails)
	gs.processStatusService.LogStep(processID, step1, constant.Success, string(constant.FetchEmailAttachment), errorMsg, nil, nil, nil, nil)
	...
	if err != nil {
		...
		gs.processStatusService.LogStepAndFail(processID, step, constant.Failure, msg, err.Error())
		log.Println("@GmailSyncCore->FetchEmailsWithAttachments:", userId, " err:", err)
		return err
	}
	gs.processStatusService.LogStep(processID, step1, constant.Success, string(constant.EmailAttachmentFetch), errorMsg, nil, nil, nil, nil)
	step3 := string(constant.ProcessSaveRecords)
	msg3 := string(constant.SaveRecord)
	totalRecord := len(emailMedRecord)
	gs.processStatusService.LogStep(processID, step3, constant.Running, msg3, errorMsg, nil, &totalRecord, nil, nil)
	...
	if saveRecordErr != nil {
		...
		gs.processStatusService.LogStepAndFail(processID, step3, constant.Failure, string(constant.FailedSaveRecords), saveRecordErr.Error())
		return saveRecordErr
	}
	gs.processStatusService.LogStep(processID, step3, constant.Success, msg, errorMsg, nil, nil, nil, nil)
	step4 := string(constant.ProcessDigitization)
	msg4 := string(constant.DigitizationTaskQueue)
	gs.processStatusService.LogStep(processID, step4, constant.Running, msg4, errorMsg, nil, nil, nil, nil)
	...
	if err != nil {
		gs.processStatusService.LogStepAndFail(processID, step, constant.Failure, string(constant.UserProfileNotFound), err.Error())
	...
	for idx, record := range emailMedRecord {
		if record.RecordCategory == string(constant.TESTREPORT) || record.RecordCategory == string(constant.PRESCRIPTION) {
			...
			gs.processStatusService.LogStep(processID, step4, constant.Running, msg, errorMsg, &record.RecordId, nil, nil, nil)
			...
			if taskErr != nil {
				...
				gs.processStatusService.LogStepAndFail(processID, step4, constant.Failure, "Record digitization failed", taskErr.Error())
			}
			gs.processStatusService.LogStep(processID, step4, constant.Success, msg, errorMsg, &record.RecordId, nil, nil, nil)
		}
	}
	...
	gs.processStatusService.LogStep(processID, step4, constant.Success, msg5, errorMsg, nil, nil, nil, nil)
	return nil
}
...
func (s *GmailSyncServiceImpl) SyncGmailWeb(userID uint64, code string) error {
	...
	if err != nil {
		s.processStatusService.LogStepAndFail(processIdKey, step, constant.Failure, string(constant.TokenExchangeFailed), err.Error())
		return err
	}
	s.processStatusService.LogStep(processIdKey, step, constant.Success, string(constant.TokenExchangeSuccess), errorMsg, nil, nil, nil, nil)
	...
	if tokenErr != nil {
		s.processStatusService.LogStepAndFail(processIdKey, step, constant.Failure, "Failed to save user token: ", tokenErr.Error())
	} else {
		s.processStatusService.LogStep(processIdKey, step, constant.Success, "User token fetch successfully", "", nil, nil, nil, nil)
	}
	...
	if err != nil {
		s.processStatusService.LogStepAndFail(processIdKey, step, constant.Failure, string(constant.GmailClientCreateFailed), err.Error())
		return err
	}
	s.processStatusService.LogStep(processIdKey, step, constant.Success, string(constant.GmailClientCreated), errorMsg, nil, nil, nil, nil)
...
func (s *GmailSyncServiceImpl) SyncGmailApp(userID uint64, gmailService *gmail.Service) error {
	...
	errorMsg := ""
	s.processStatusService.LogStep(processID, string(constant.GmailSync), constant.Running, msg, errorMsg, nil, nil, nil, nil)
