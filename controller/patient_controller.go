...
func (pc *PatientController) CreateShareLink(ctx *gin.Context) {
	_, userID, _, err := utils.GetUserIDFromContext(ctx, pc.userService.GetUserIdBySUB)
	if err != nil {
		models.ErrorResponse(ctx, constant.Failure, http.StatusUnauthorized, err.Error(), nil, err)
		return
	...
	var req struct {
		Password string `json:"passkey" binding:"required"`
	}
	if err := ctx.ShouldBindJSON(&req); err != nil {
		models.ErrorResponse(ctx, constant.Failure, http.StatusBadRequest, "invalid request", nil, errors.New("please check request body"))
		return
	...
	userDetails, err := pc.patientService.GetUserProfileByUserId(userID)
	if err != nil {
		models.ErrorResponse(ctx, constant.Failure, http.StatusUnauthorized, "Failed to load profile", nil, err)
		return
	}
	resp, err := pc.userService.CreateSharedProfileLink(userID, userDetails.Email, req.Password)
	if err != nil {
		models.ErrorResponse(ctx, constant.Failure, http.StatusInternalServerError, "Failed to create shared link", nil, err)
		return
	}

	models.SuccessResponse(ctx, constant.Success, http.StatusOK, "Shared link created successfully", resp, nil, nil)
	return
}

func (pc *PatientController) GetSharedProfile(ctx *gin.Context) {
	linkId := ctx.Param("link_id")
	if linkId == "" {
		models.ErrorResponse(ctx, constant.Failure, http.StatusBadRequest, "Missing query parameters", nil, nil)
		return
	}
	resp, err := pc.userService.GetSharedProfileLink(linkId)
	if err != nil {
		models.ErrorResponse(ctx, constant.Failure, http.StatusInternalServerError, "Failed to verify link", nil, err)
		return
	}
